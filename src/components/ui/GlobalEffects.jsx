/**
 * GlobalEffects.jsx — Optimized v2
 * 
 * Fix:
 *  - Kursor normal di sidebar (cursor: none HANYA di canvas)
 *  - NoiseGrain dihapus (terlalu berat, canvas per-pixel)
 *  - CursorGlow hanya aktif ketika mouse di atas canvas/scene
 *  - AmbientOrbs: cukup CSS, tidak perlu JS
 *  - CornerHUD: lebih ringan
 */
...
function CursorGlow() {
  ...
  useEffect(() => {
    ...
    attachToCanvas()
    // Retry setelah mount (canvas mungkin belum ada)
  ...
  // Sembunyikan kalau tidak di atas canvas
  ...
  return (
    <>
      <div
        ref={dotRef}
        className="hao-cursor-dot"
        style={{
          position: 'fixed',
          top: 0, left: 0,
          width: 8, height: 8,
          borderRadius: '50%',
          background: 'rgba(99,184,255,0.9)',
...
// ── Corner HUD decorations — pure CSS, no JS ──────────────────────────────
function CornerHUD() {
  const corners = [
...
// ── Ambient orbs — pure CSS, no JS, GPU-only via transform ───────────────
